...
# DNS linking handled inside the jump server module 
module "jump_server" {
  ...
  # DNS linking parameters 
  vnet_id                 = module.networking.hub_vnet_id
  dns_zone_name           = join(".", slice(split(".", module.aks.cluster_private_fqdn), 1, length(split(".", module.aks.cluster_private_fqdn))))
  dns_zone_resource_group = module.aks.node_resource_group

  # AUTOMATION CREDENTIALS (No manual login!)
...
#   # DNS linking parameters (like your working example)
#   vnet_id                     = module.networking.hub_vnet_id
#   dns_zone_name               = join(".", slice(split(".", module.aks.cluster_private_fqdn), 1, length(split(".", module.aks.cluster_private_fqdn))))
#   dns_zone_resource_group     = module.aks.node_resource_group
